...
internal class HttpGetSendFeature(IHttpClientProvider httpClientProvider) : ISendFeature
{
    public string Key { get; } = "HTTP_GET";
    ...
    public async Task<SchismResponse> SendAsync(SchismRequest request, ConnectionPoint point)
    {
        ...
        HttpResponseMessage response = await httpClient.GetAsync($"{request.BaseUri.TrimEnd('/')}/{point.Path}?{query}");
