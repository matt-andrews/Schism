...
internal class HttpHeadSendFeature(IHttpClientProvider httpClientProvider) : ISendFeature
{
    public string Key { get; } = "HTTP_HEAD";
    ...
    public async Task<SchismResponse> SendAsync(SchismRequest request, ConnectionPoint point)
    {
        ...
        HttpResponseMessage response = await httpClient.HeadAsync($"{request.BaseUri.TrimEnd('/')}/{point.Path}?{query}");
