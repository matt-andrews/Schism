...
internal class HttpPostSendFeature(IHttpClientProvider httpClientProvider) : ISendFeature
{
    public string Key { get; } = "HTTP_POST";
    ...
    public async Task<SchismResponse> SendAsync(SchismRequest request, ConnectionPoint point)
    {
        ...
        HttpResponseMessage response = await httpClient.PostAsync($"{request.BaseUri.TrimEnd('/')}/{point.Path}?{query}", jsonBody);
