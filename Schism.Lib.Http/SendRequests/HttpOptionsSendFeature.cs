...
internal class HttpOptionsSendFeature(IHttpClientProvider httpClientProvider) : ISendFeature
{
    public string Key { get; } = "HTTP_OPTIONS";
    ...
    public async Task<SchismResponse> SendAsync(SchismRequest request, ConnectionPoint point)
    {
        ...
        HttpResponseMessage response = await httpClient.OptionsAsync($"{request.BaseUri.TrimEnd('/')}/{point.Path}?{query}");
