...
internal class TestHttpHeadSendFeature
{
    [Test]
    public async Task HeadRequestWithQueryParamsString()
    {
        ...
        client.HeadAsync(Arg.Any<string>()).Returns(Task.FromResult(new HttpResponseMessage() { StatusCode = System.Net.HttpStatusCode.OK }));
        HttpHeadSendFeature feature = new(clientProvider);
        ...
        _ = client.Received(1).HeadAsync(Arg.Is<string>(i => i == "baseuri/path?A=12&B=8"));
    }
    [Test]
    public async Task HeadRequestWithQueryParamsJsonBody()
    {
        ISchismHttpClient client = Substitute.For<ISchismHttpClient>();
        IHttpClientProvider clientProvider = Substitute.For<IHttpClientProvider>();
        clientProvider.GetHttpClient(Arg.Any<SchismRequest>()).Returns(client);
    ...
    [Test]
    public async Task HeadRequestWithQueryParamsJsonBodyAndString()
    {
        ISchismHttpClient client = Substitute.For<ISchismHttpClient>();
        IHttpClientProvider clientProvider = Substitute.For<IHttpClientProvider>();
        clientProvider.GetHttpClient(Arg.Any<SchismRequest>()).Returns(client);
        ...
        _ = client.Received(1).HeadAsync(Arg.Is<string>(i => i == "baseuri/path?C=4&D=2&A=12&B=8"));
    }
}