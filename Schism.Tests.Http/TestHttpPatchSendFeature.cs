...
internal class TestHttpPatchSendFeature
{
    [Test]
    public async Task PatchRequestWithJsonBody()
    {
        ...
        client.PatchAsync(Arg.Any<string>(), Arg.Any<object>())
            .Returns(Task.FromResult(new HttpResponseMessage()
            {
                StatusCode = System.Net.HttpStatusCode.OK,
                Content = new StringContent(JsonSerializer.Serialize(responseData))
            }));
        HttpPatchSendFeature feature = new(clientProvider);
        ...
        _ = client.Received(1).PatchAsync(Arg.Is<string>(i => i == "baseuri/path?"), responseData);
    ...
    [Test]
    public async Task PatchRequestWithQueryParamsJsonBodyAndString()
    {
        MockResponse responseData = new(12, 8);
        ISchismHttpClient client = Substitute.For<ISchismHttpClient>();
        IHttpClientProvider clientProvider = Substitute.For<IHttpClientProvider>();
        ...
        Assert.That(result.StatusCode, Is.EqualTo(System.Net.HttpStatusCode.OK));
        //JsonBody is appended to query string list
        _ = client.Received(1).PatchAsync(Arg.Is<string>(i => i == "baseuri/path?C=4&D=2"), responseData);
